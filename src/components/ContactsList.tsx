...
return(
   ...
   <div className="flex justify-center mt-20 ">
        <ul className='grid grid-cols-2 gap-x-5'>
            {props.contactList.map(item => <div key={item.id} onClick={() => navigate(item.id)}>
                
                    <div className='rounded border border-[#f59e0b] py-8 px-10 my-4 mx-auto'>
                    <p className='capitalize font-semibold text-base text-center'>{item.firstname} {item.lastname}</p>
